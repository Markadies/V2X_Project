...
/*Making 2 instances f the struct, and itializing them with 0*/
ring_buffer rx_buffer = { { 0 }, 0, 0};
ring_buffer tx_buffer = { { 0 }, 0, 0};

/*Making 2 pointers to the ringbuffer struct*/
...
void store_char(unsigned char c, ring_buffer *buffer)
{
	/*(i) is an index of the next head location(not this iteration)
	 * because when i assign this charachter in this location and the head
...
{
	/*strlen function stores the length of the variable inside
	 * we will store of the needed to search for string and the buffer we will look into*/
