...
String Buffer;
String Global_fGPS_Latitude  = "30.09474";
String Global_fGPS_Longitude = "31.31404";
String Global_uint16Speed    = "2";
const char* ssid = "Bendary";
const char* password = "01276047636rr";
const char* car_id = "Car1";  // Define the car ID
...
// MQTT topics
const char* pub_topic = "cars/Car1/data";
const char* sub_topic = "cars/Car1/commands";
const char* pub_key_topic = "cars/public_keys";

WiFiClient Car1;
PubSubClient client(Car1);
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("Car1")) {
      Serial.println("connected");
      if (client.subscribe(sub_topic)) {
        Serial.println("Subscribed to topic: cars/Car1/commands");
      } else {
...
void onUARTReceive() {
  ...
  if (SerialPort.available()) {
    ...
    if(Local_uint32TimeOutErrorCode == false) {
      if(Buffer[i] == '%') {
        if(Buffer[i+1] == 'L') {
          dimLights_detect = true;
          publish_Ready = true;
        } else if(Buffer[i+1] == 'K') {
          overtaking_request = true;
          publish_Ready = true;
        }
      } else if(Buffer[i] == '#') {
...
void publishPublicKey() {
  String CarpublicKey = readKey("/car1_public_key.pem");
...
void decryptMessage(const String& encryptedBase64, const String& privateKey) {
    ...
    if (strcmp(event, "hardBraking") == 0) {
        ...
        //Send 'B' to STM32 if hard braking detected
         SerialPort.write('B');
         SerialPort.write('B');
    ...
    } else if (strcmp(event, "overtaking_approved") == 0) {
        ...
        // Add your code here to handle overtaking_request
    }
       else if (strcmp(event, "overtaking_denied") == 0) {
        ...
        // Add your code here to handle Dim_Light
    } else {
...
void publishData() {
  ...
  dimLights_detect = false;
  overtaking_request = false;
  publish_Ready = false;
}
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (chunkIndex <= MAX_CHUNKS) {
    ...
    if (receivedCount == totalChunks) {
      ...
      // Read private key
      String privateKey = readKey("/car1_private_key.pem");
